...
<body>
    ...
    <section>
        <article> <?php
                    ...
                    echo "</p>\n</div>\n";
                    $sentencia->close();    /*  $consulta  = "INSERT INTO libros (isbn, autor, titulo, precio) "; $consulta .= "VALUES ('" . $isbn . "', '" . $autor . "', '" . $titulo . "', " . $precio .  ")"; $resultc = $db->query($consulta); if($resultc){ echo $db->affected_rows . " libro agregado a la base de datos."; } */ //Cerrar la conexión $db->close(); 
                    ?> <br /> <a href="nuevolibro.html" class="a-btn"> <span class="a-btn-symbol">i</span> <span class="a-btn-text">Agregar</span> <span class="a-btn-slide-text">otro libro</span> <span class="a-btn-slide-icon"></span> </a> <a href="menuopciones.html" class="a-btn"> <span class="a-btn-symbol">i</span>